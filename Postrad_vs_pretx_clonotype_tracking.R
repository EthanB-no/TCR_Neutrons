...
clonotype_annotated <- clonotype_data %>%
  ...
  filter(Timepoint %in% c("Pre-TX", "Post Rad"))
...
for (pid in patient_ids){
  ...
  if (!all(c("Clones_Pre-TX", "Clones_Post Rad") %in% colnames(clonotype_tracked_patient))) {
    message("Skipping patient ", pid, ": missing Post Rad or Pre-TX")
    next
  }
  ...
  clonotype_analysis_patient <- clonotype_tracked_patient %>%
    mutate(
      ...
      FC = log2((`Clones_Post Rad` + 1) / (`Clones_Pre-TX` + 1)),
      
      Status = case_when(
        FC > log2(1.5) & `Clones_Pre-TX` >= 1 & `Clones_Post Rad` >= 1 ~ "Expanded (FC > 1.5)",
        FC < log2(0.5) & `Clones_Pre-TX` >= 1 & `Clones_Post Rad` >= 1 ~ "Contracted (FC < 0.5)",
  ...
  max_val <- max(clonotype_analysis_patient$Log_Clones_PreTX,
                 clonotype_analysis_patient$Log_Clones_PostRad)
  
  scatter_plot_colored <- clonotype_analysis_patient %>%
    ggplot(aes(x = Log_Clones_PreTX, y = Log_Clones_PostRad, color = Status)) +
    geom_point(alpha = 0.6, size = 2.5) +
    geom_abline(intercept = 0, slope = 1, linetype = "dashed",
                color = "gray50", linewidth = 0.8) +
    labs(
      title = paste("Clonotype Tracking (Pre-TX vs Post Rad) Patient", pid),
      x = expression(log[10]~"(Pre-TX Clones + 1)"),
      y = expression(log[10]~"(Post Rad Clones + 1)"),
...
wilcox.test(per_patient_summary$`Expanded/New`,
            per_patient_summary$`Contracted/Lost`,
            paired = TRUE)
...
p1 <- ggplot(status_summary, aes(x = FateGroup, y = Count, fill = Subgroup)) +
  ...
  labs(
    title = "Cohort-level Clonotype Fate: Post Rad vs Pre-TX",
...
ggsave(
  filename = "cohort_level_clonotype_fate_POSTRAD.png",
  plot = p1,
  dpi = 400,
  width = 6,
  height = 6,
